...
// TestFirstRepeatCmd_Execute тест команды, которая повторяет Команду, выбросившую исключение.
func TestFirstRepeatCmd_Execute(t *testing.T) {
	type fields struct {
		err error
	}
	tests := []struct {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			f := commands.NewFirstRepeatCmd(mockExecute, tt.fields.err)
